...
function home(t_sens1,base,t_sens2,incl,claw)               % Homing function to go to home position
...
function locate(base,theta)                                 % Station Locating function
    rot_base = 3.3 * theta;
    if(rot_base >= (-readRotation(base)))
        while(rot_base >= (-readRotation(base)))
            readRotation(base);
...
function pick(incl,claw,t_sens2,theta2)                      % Picking function
    resetRotation(incl)
...
function place(incl,claw,t_sens2,theta2)                    % Placing function
    resetRotation(incl);
...
function theta_2 = calc_inv(Z)                          % Inverse kinematics function
    O_ffset = 55;
    l0 = 70;
    l1 = 50;
    l2 = 95;
